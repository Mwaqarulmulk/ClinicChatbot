...
const envSchema = z
  ...
  .superRefine((env, ctx) => {
    const remoteTurso = env.TURSO_DATABASE_URL.startsWith("libsql://") || env.TURSO_DATABASE_URL.startsWith("wss://");
    if (remoteTurso && !env.TURSO_AUTH_TOKEN) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["TURSO_AUTH_TOKEN"],
        message: "TURSO_AUTH_TOKEN is required when TURSO_DATABASE_URL points to a remote Turso database.",
      });
    }
  });
