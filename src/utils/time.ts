...
export function parseLooseDateTime(text: string, now = new Date(), timeZone?: string): Date | null {
  ...
  if (timeZone) return zonedTimeToUtc({ year, month, day, hour, minute }, timeZone);
...
export function zonedBusinessTime(date: Date, timeZone: string, hour: number, minute = 0): Date {
  const parts = getZonedParts(date, timeZone);
  return zonedTimeToUtc({ year: parts.year, month: parts.month, day: parts.day, hour, minute }, timeZone);
}
...
function zonedTimeToUtc(parts: ZonedParts, timeZone: string): Date {
  ...
  for (let iteration = 0; iteration < 2; iteration += 1) {
    ...
    const zonedAsUtc = Date.UTC(zoned.year, zoned.month - 1, zoned.day, zoned.hour, zoned.minute, 0, 0);
    const intendedAsUtc = Date.UTC(parts.year, parts.month - 1, parts.day, parts.hour, parts.minute, 0, 0);
